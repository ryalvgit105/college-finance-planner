...
const BudgetSection = ({ category, title, description, items, onAddItem, onDeleteItem, isEditing }) => {
    ...
    return (
        <div className="bg-gradient-to-b from-slate-800/80 to-slate-900/70 p-4 rounded-xl border border-slate-700/80 flex flex-col">
            ...
            {isEditing && (
                <form onSubmit={handleSubmit} className="flex gap-2 mt-auto">
                    <input
                        ...
                        className="flex-grow bg-slate-800 border border-slate-600 rounded-md p-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 transition-shadow"
                    />
                    <input
                        type="number"
                        placeholder="Amt"
                        ...
                        className="w-20 bg-slate-800 border border-slate-600 rounded-md p-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 transition-shadow"
                    />
                    <button type="submit" className="bg-sky-600 hover:bg-sky-500 text-white font-bold px-3 rounded-md text-sm transition-colors button-glow-sky">
                        Add
                    </button>
                </form>
            )}
            ...
            <div className="text-right font-bold mt-4 border-t border-slate-700 pt-3">
                <span className="text-slate-300">Category Total: </span>
