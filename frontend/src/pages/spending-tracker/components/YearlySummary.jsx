...
const YearlySummary = ({ expensesByMonth, totalIncome, year }) => {
    const totalAnnualSpending = Object.values(expensesByMonth).reduce((sum, spent) => sum + spent, 0);
    const totalAnnualIncome = totalIncome * 12;
    const annualNetFlow = totalAnnualIncome - totalAnnualSpending;

    const monthsWithSpending = Object.values(expensesByMonth).filter(spent => spent > 0).length;
    const averageMonthlySpending = monthsWithSpending > 0 ? totalAnnualSpending / monthsWithSpending : 0;

    const maxMonthlySpending = Math.max(...Object.values(expensesByMonth), 1); // Avoid division by zero, min value 1
        ...
        <div className="mt-10 bg-slate-900 p-6 rounded-2xl border border-slate-800">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-center">
                ...
                <div className="lg:col-span-3">
                    ...
                    <div className="flex justify-between items-end h-56 bg-slate-950/50 p-4 rounded-xl space-x-2 border border-slate-800">
                        {MONTH_NAMES.map((month, index) => {
                            ...
                            const barHeight = maxMonthlySpending > 0 ? (spent / maxMonthlySpending) * 100 : 0;
                            return (
                ...
                <div className="space-y-4 text-base lg:col-span-2 tabular-nums">
                    ...
                    <div className="flex justify-between items-center p-4 bg-gradient-to-r from-sky-500/10 to-transparent rounded-lg mt-2 border border-sky-500/20">
